...
### B.11 Claude descriptions module (auto-fill city/neighborhood)
...
a single short Hebrew paragraph generated by Claude (`claude-haiku-4-5`).
The system prompt enforces qualitative-only output (no quantitative
claims) per A.3 and `skills/03_description.md`.

Wiring: `real_estate/web.py:generate(...)` accepts three new optional
form fields — `city_description`, `neighborhood_name`,
`neighborhood_description`. When `city_description` is empty (or contains
the legacy "יש להשלים" placeholder) and a city was parsed from the
address, `try_describe_city(city)` is called and the result, suffixed
with the source footnote (`with_footnote(...)`), replaces the placeholder.
The neighborhood path mirrors this and additionally requires the
appraiser to provide a neighborhood name (no other source for it yet).

Failures are non-fatal: `try_describe_city` / `try_describe_neighborhood`
log a warning and return `None`, leaving the existing "יש להשלים"
placeholder in place so the report is still produced. This protects
the appraiser from API outages, missing keys, and quota issues.
